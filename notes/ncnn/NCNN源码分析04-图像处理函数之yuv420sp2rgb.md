yuv420sp包括了NV21 (Y plane + VU交错plane)、NV12（Y plane + UV交错plane），NV21和NV12的差异在于u和v的顺序交换。NCNN支持三种类型的yuv420sp转rgb，分别是：NV21转rgb、NV12转rgb、NV21转rgb + half resize

## NV21转rgb
这里的yuv420sp2rgb采用的转换公式如下，这里输入的YUV是2通道，2个Y分量共用一组UV。
* R = ((Y << 6) + 90 * (V-128)) >> 6
...
void yuv420sp2rgb(const unsigned char* yuv420sp, int w, int h, unsigned char* rgb)
{
    ...
    for (int y = 0; y < h; y += 2)
    {
        ...
        for (; nn > 0; nn--)
        {
            // uint8x8_t -> 左移6位生成uint16x8_t，Y << 6 >> -> 类型强转为int16x8_t
            ...
            // uint8x8_t -> V-128 -> 类型强转为int8x8_t
            int8x8_t _vvuu = vreinterpret_s8_u8(vsub_u8(vld1_u8(vuptr), _v128));
            //  第一个输入向量中的奇数索引的元素与第二个输入向量中的偶数索引的元素交换
            // vuvuvuvu -> vvvv uuuu
...
yuv420sp2rgb_nv12的实现同yuv420sp2rgb基本一致，区别是VU顺序调整为UV。
...
## NV21转rgb + half resize

yuv420sp2rgb_half的实现类似yuv420sp2rgb基本一致，区别是输出rgb图像按w/2、h/2尺寸，这里输入的YUV是1.5通道，4个Y分量共用一组UV。
