...
class Tensor(Value):
    ...
    def backward(self, out_grad=None):
        # print("292:", self, out_grad is None, self.dtype)
        out_grad = (
            out_grad
            if out_grad
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    # print("380 out_grad:", type(out_grad), out_grad.dtype)
    ...
    for node in reverse_topo_order:
      ...
      for grad in node_to_output_grads_list[node]:
        ...
        # 20250203 重要，否则出现float64
        if node.grad is None:
          node.grad = grad
        else:
          node.grad = node.grad + grad
        # print(grad.dtype, type(node), node.grad.dtype, node.grad.shape)
      if node.op is not None:
        node_back_grad = node.op.gradient_as_tuple(node.grad, node)
        for idx in range(len(node.inputs)):
          input_node = node.inputs[idx]
          if input_node in node_to_output_grads_list.keys():
            node_to_output_grads_list[input_node].append(node_back_grad[idx])
          else:
            node_to_output_grads_list[input_node] = [node_back_grad[idx]]     
      #break
    ### BEGIN YOUR SOLUTION
    # raise NotImplementedError()
    ### END YOUR SOLUTION
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for node in node_list:
      cur_node = node
      while len(q) > 0 or (cur_node not in node2idx.keys()): 
        while (cur_node not in node2idx.keys()):
          node2idx[cur_node] = 0
          q.append(cur_node)
          if len(cur_node.inputs) == 0:
            break
          cur_node = cur_node.inputs[0]
        last_node = q[-1]
        if len(last_node.inputs) == 0 or len(last_node.inputs) == node2idx[last_node] + 1:
          ret.append(last_node)
          q.pop(-1)
        else:
          node2idx[last_node] += 1
          cur_node = last_node.inputs[node2idx[last_node]]
    return ret
    ### END YOUR SOLUTION
